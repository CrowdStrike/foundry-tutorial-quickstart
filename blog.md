...
- Install: Install a relased app via the App manager. 
...
For Sockets, select *Endpoint detection details*. 

![foundry ui extensions create](images/foundry-ui-extensions-create.png)

// Explain how sockets are like websockets and how you can also develop pages.

Sockets are predefined locations within the Falcon console where your extension can be rendered. Selecting Endpoint detection details means that your app will be rendered on the /activity-v2/detections/ page in the Falcon console. When a Falcon console user selects an endpoint detection to see details, your app data is displayed in this panel.
...
The Foundry platform is in development mode, which means it has an open WebSocket connection to your local machine. This means you can edit `ui/extensions/My First Extension/src/app.js` in your favorite editor, and you can see the changes right away. For example, I changed mine to say "Hello, Falcon Foundry!", then right-clicked on the extension in Chrome and selected **Reload Frame**. My changes are viewable right away. Pretty slick, eh?
...
- **UI Capabilities:** Create extensions for display in detail panels, including CrowdScore incidents, endpoint detections, host management hosts, <abbr title="Extended Detection and REsponse">XDR</a> detections, and workflow executions. You can also create standalone pages that are added to Falcon's navigation menu.
